...
> virtual dom只做了简单的事，就是描述整体的dom结构，标签，属性，子节点，父节点，这些一系列的属性，只是个内存中对象。而如果直接去创建dom，那一定也是要有一个对应的内存对象来描述dom的，而且创建的对象的key比vnode的多很多，那初始化也要相对时间更久，占用内存更高。如果算上将dom添加到document上，那就经过一系列的对比更新，导致浏览器页面的重新渲染。所以这么比较两者的效率根本没有意义啊，vdom做一件事就是描述，dom做两件事，描述+更新，用一件事跟两件事比较效率，毫无意义。如果dom不添加到document上，那所有的浏览器肯定也不会触发重渲染，那vdom的优势其实就是描述起来简单，初始化简单，占用内存可能更少而已。说vdom有优势，那应该说是利用vdom去描述页面，并进行计算之后，得出最后页面的状态描述，然后一次性更新需要更新的，比dom不断地更新有优势。
